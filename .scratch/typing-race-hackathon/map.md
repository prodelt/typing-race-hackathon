...
- [01 Task: Clean public repo baseline](issues/01-clean-public-repo-baseline.md): commit `2dcaba0` pushed to `prodelt/typing-race-hackathon` as `main` — 75 files, one clean initial commit, no vendored skills, no organizer data, secret scan clean, author email hidden behind the GitHub noreply address. Research branches folded into `docs/research/`. Branch protection recorded for the first code task.

- [14 Grilling: Live races & group leaderboards](issues/14-races-and-group-leaderboards.md): quick match and private-code rooms of up to 5, latecomers and idlers become spectators; one shared ~300-character text from our own corpora; Broadcast carries progress twice a second for display only, the start is anchored to a server timestamp, and an Edge Function replays the keystroke log against the race text before any result is ranked; accuracy-weighted winner with a 90% accuracy floor; group, weekly and global boards from summary tables.

- [13 Grilling: Accounts, sync & privacy](issues/13-accounts-sync-privacy.md): email + password with confirmation off (instant profile for the demo) plus Google/GitHub OAuth, PKCE flow, Resend only for resets ([ADR-0005](../../docs/adr/0005-append-only-attempts-as-source-of-truth.md)); attempts are append-only and progress is derived server-side, so cross-device conflicts cannot occur; full keystroke logs kept for the last 20 attempts and pruned after 30 days, aggregates forever; IndexedDB cache plus an outbox queue makes §4.1/§8.9 pass offline; mandatory nickname, no avatars, no third-party analytics, self-service deletion via an Edge Function.
