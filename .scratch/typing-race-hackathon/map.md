...
- [01 Task: Clean public repo baseline](issues/01-clean-public-repo-baseline.md): commit `124c444` pushed to `prodelt/typing-race-hackathon` as `main` — 75 files, one clean initial commit, no vendored skills, no organizer data, secret scan clean, author email hidden behind the GitHub noreply address. Research branches folded into `docs/research/`. Branch protection recorded for the first code task.
